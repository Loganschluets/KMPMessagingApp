...
void handle_delete_messages(int client_sock, const char *recipient) {
    printf("Handling DELETE request for recipient: %s\n", recipient);

    // Open the messages file for reading
    ...
    // Temporary file to store messages that don't match the recipient
    FILE *temp_file = fopen("temp_messages.txt", "w");
    if (!temp_file) {
        fclose(file);
    ...
    // Process each line in the messages file
    while (fgets(line, sizeof(line), file)) {
        struct json_object *message_obj = json_tokener_parse(line);
        struct json_object *msg_recipient;

        if (json_object_object_get_ex(message_obj, "receiver", &msg_recipient)) {
            const char *msg_recipient_str = json_object_get_string(msg_recipient);

            // Skip writing to temp file if the recipient matches
            if (strcmp(msg_recipient_str, recipient) == 0) {
                deleted_count++;
                continue;
            }
        }
        // Write messages that don't match the recipient to the temp file
    ...
    // Replace original messages file with updated temp file
    if (rename("temp_messages.txt", MESSAGE_FILE) != 0) {
        const char *error_response = "HTTP/1.1 500 Internal Server Error\r\nContent-Type: text/plain\r\n\r\nFailed to update message file.";
        send(client_sock, error_response, strlen(error_response), 0);
        return;
    }
    ...
    snprintf(success_response, sizeof(success_response),
             "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\n\r\nDeleted %d messages for recipient: %s.", deleted_count, recipient);
    send(client_sock, success_response, strlen(success_response), 0);
    printf("Deleted %d messages for recipient: %s\n", deleted_count, recipient);
}
...
int main() {
    ...
    while (1) {
        ...
        } else if (strncmp(buffer, "DELETE /deleteMessages", 22) == 0){
            
            char *recipient_param = strstr(buffer, "recipient=");

            if(recipient_param != NULL) {

                recipient_param += 10;

                char *recipient_end = strstr(recipient_param, " ");

                if(recipient_end != NULL){
                    *recipient_end = '\0';
                }
                handle_delete_messages(client_sock, recipient_param);
            }else{
                const char *error_response = "HTTP/1.1 400 Bad Request\r\nContent-Type: text/plain\r\nRecipient paramter missing.";
                send(client_sock, error_response, strlen(error_response), 0);
            }
        } else {
